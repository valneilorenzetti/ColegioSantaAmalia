	...
	<section class="row center-int">
		<section class="large-8 medium-8 columns text-int">
			<?php if(get_post_type( $post ) == "post"){ ?>
				<h1 class="tit-int"><?php echo qtrans_useCurrentLanguageIfNotFoundUseDefaultLanguage('[:pt]Notícia[:en]New'); ?></h1>
				<h2><?php the_title(); ?></h2>
				<?php the_content(); ?>
				<?php
				
					if(get_bloginfo('language') == "en-US"){
					     $lang_url = get_bloginfo('url')."/en/noticias/";
					}
					else{
					     $lang_url =  get_bloginfo('url')."/noticias";
					}
				?>
				<a href="<?php echo $lang_url; ?>"><?php echo qtrans_useCurrentLanguageIfNotFoundUseDefaultLanguage('[:pt]Ver todas as notícias[:en]See all news'); ?></a>
			<?php }else{ ?>
